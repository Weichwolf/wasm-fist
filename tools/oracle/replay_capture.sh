...
# render), 0x13487 (the 22f7 phase dispatch), 0x1d7e9 (c659, a shell drawn: ESI = the shell), the
# extender's LOS service op 0x58 at 0x1000805a (EBX/EDX/EAX = the candidate-object deltas) and its
# return 0x10001108 (EAX = the verdict) -- and
...
# [OC_EXTRA_LIN=<hex>,<hex>]  more seams for one investigation (the regtrace prints the registers at each)
set -e
...
exec env OC_WALL="${OC_WALL:-45}" \
  ...
  FIST_REGTRACE_LIN="11447,11454,24288,1d275,24ab0,158e4,1346d,13487,1d7e9,1000805a,10001108${OC_EXTRA_LIN:+,$OC_EXTRA_LIN}" \
