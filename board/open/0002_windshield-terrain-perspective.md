...
## Merged-in work log (was a headerless duplicate at board/active/0002_windshield-terrain-perspective.md)

The board convention is one file per item and the path IS the state; this log had been appended
to a second, header-less file under board/active/ carrying the same number, so item 0002 existed in
two directories at once.  Folded back in verbatim, nothing dropped; the item stays open.
