...
## Status — **STAGE 3 SUB-SCREEN OPEN: clicking "ABOUT FIST" now OPENS its dialog — the full event→activate→screen-state-machine→modal-loop chain works and the dialog PANEL (frame + button row) renders crash-free + DETERMINISTICALLY on native.** 2 new patches (129/130) + 1 shim fix (`re_out/fist_dos.c` DOS 8.3 space-pad canonicalization). Menu UNCHANGED — `verify.sh both` = PASS mainmenu (native↔wasm 0-diff, md5 `3a6ff1c5`, AE=0 vs DOSBox); ABOUT panel native md5 `49310ace` (deterministic). `re_out/fist.c` pristine (`61453e42`), 130 patches apply. **The chain:** `FIST_MOUSE` click on ABOUT FIST (elem 3b60, row 139) → `39d7→2f03→349b enqueue→35a7 drain→1e4b→fist_hit1eb4` → activate method `[elem+0x4660]=ea5a` writes screen-state `DGROUP:0xf78e=0xc` → e714 state jumptable (`0xc→0xe969=e39f();7088(4)`) → the **modal dialog loop 7088** loads FLDCOMP.MRL (the generic dialog panel, idx 0x20 via 71de) + spins. Reconstructed all base-loss/arg-drop on the path (ea5a activate + the 3 `[elem+0x4660]` dispatch-arg threads; 7088 builder-dispatch + CF flow + cooperative pump; 71de bg-load; 23bf/92ea; driver 0fbc/0fe4). **The DOS 8.3 name is space-PADDED ("FLDCOMP .MRL") — the shim `open_ci` now strips the pad; that was the actual load-failure root cause** (distinct-indices 256→44). New tool `tools/refcapture_click.sh` + reference `ref/about_native320.png` (scripted-click DOSBox via XTest). **FRONTIER — the credit TEXT + wasm dialog parity:** the panel renders but the credit-text elements do NOT (they paint via the `[+0x4872]` multi-element walk `1e88` which patch 116 hard-stops after the first element; walking all needs per-element paint-method CF+arg threading + reconstructing base-lost methods e.g. `ea7e`), so it is NOT yet bit-identical to DOSBox; and the wasm click path OOBs (bounds-checked wasm vs native's 1 MB g_mem, a modal-loop timing divergence). The MENU flow is unaffected by both. Full detail: `docs/stage1.md` (top). Prior status:
