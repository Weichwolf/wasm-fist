...
> **⚠️ THIS STATUS SECTION IS STALE (stops at patch 153; the repo is at patch 343 / 26 verify.sh flows).**
> For the ACCURATE current-state frontier map, read **`docs/STATE.md`** (2026-07-18): 26 bit-verified flows
> (entire front-end — intro, all menus, all SETTINGS toggles, all list-dialog interactions, OK-save,
> campaign/battle drill-down); mission ACCEPT cascade crash-free but unverified; cockpit/HUD reconstructed
> (~AE 6209 residual); mission windshield renders the correct scene behind `FIST_TILEFILL` (WHOLE AE 27325,
> bit-exact blocked on the 6980 render-time-paging coord-layout deep frontier — see the
> `mission-terrain-tile-pipeline` memory + `docs/oracle_terrain_*.md`); **genuinely UNTOUCHED: audio playback
> (half the DoD), the level/mission editor round-trip, other missions/maps, gameplay-in-motion, save/load.**
> The detailed patch-153-era narrative below is kept for its patch rationale only.

## Status (patch-153 era, superseded by docs/STATE.md) — **STAGE 3 (THE BRIEFING MODAL'S ACCEPT/CANCEL BUTTONS ARE RECONSTRUCTED + FUNCTIONAL, AND THE MISSION-START CASCADE FRONTIER IS PINPOINTED): clicking CANCEL in the BATTLES→OK briefing tears the dialog down and RESTORES THE MENU BIT-IDENTICALLY — AE = 0/64000 vs a genuine, reproducible DOSBox capture on BOTH native AND wasm, native↔wasm 0-diff (md5 `21ce397f`), DETERMINISTIC (tick-pinned, 5× serial identical). One new verify.sh flow → `bash tools/verify.sh both` = 19/19.** 2 new patches (152/153). `re_out/fist.c` pristine (`61453e42`); `make check` = 153 patches apply; native + wasm green. NO REGRESSION (18 prior flows PASS); `git status` clean after verify. **KEY FINDING — the ACCEPT cascade was never wired:** the real `FUN_0000_e87a` holds the full mission cascade (`e43f/4754/459a/1631/e4bb`) but **e714 does NOT call it** — patch 151 reconstructed the BATTLES dispatch INLINE and stops at `7088(0)` (briefing); after it returns (ACCEPT or CANCEL) it just sets `b78e=10`→menu, so the cascade tail is absent from the executed path (ACCEPT registers `4be2=1` but returns to the menu with zero post-briefing opens). **PATCH 152 — `FUN_0000_7217`** (briefing confirm-button activate, asm 0x7217-0x7248): `bx`=elem / `bp`=dirty-base are DGROUP NEAR OFFSETS Ghidra deref'd as host pointers (SIGSEGV fault 0x3b32); rebased to `g_mem+0x1c000+(uint16)off` + threaded the dropped CF + the dropped confirm-dispatch AX=`word[base+si+1]` (0=OK/1=CANCEL → id=0 vector `word[DGROUP:0x8c5b]=0x7249` sets `DAT_2000_4be2`). **PATCH 153 — the 7088 EXIT display-list RESTORE `1cdb`** (asm 0x70e5..0x70ed): the popped AX = the SETUP 1cdb's return HANDLE, so the restore is `1cdb(iVar2,0x8cee,0x94,0x10,param_2,uVar8)` (proven idiom, cf. patch 142); patch 151 had emitted a scrambled `1cdb(0,0x10,0x8cee,iVar2,…)` (null handle → paint walker `201a` dispatched a corrupted element → `FUN_0000_9a27` base-loss SIGSEGV fault 0x9ae4). Both exit paths are hit only by closing a modal (no prior flow does) → zero regression. **Flow `battles-cancel-briefing`** (`ref/battles_cancel_briefing_native320.png`, `tools/refcapture_ok3.sh 160 100 205 128 78 186 40 8 8`, 3 DOSBox captures AE=0 non-circular): BATTLES→OK→briefing→CANCEL(78,186)→menu; TICK-PINNED `tick=2600` (FIST_DUMPTICK, campaign-missions precedent). **CASCADE FRONTIER (pinpointed by throwaway-wiring the ACCEPT tail + tracing; NOT committed — the LOADING deliverable):** ACCEPT→`e43f`→**opens `LOADING.MS3`**→ blocks at **`FUN_0000_e459` (LOADING TITLE text, asm 0xe48d..0xe4b8)**: the `a358`/`a370` centered-string draw base-losses the STRSEG string (asm `movzbw byte[DGROUP:0xf75f]; word[DGROUP:(bx-0x8a0)]`, ES=STRSEG=`word[DGROUP:0x70]`; `a358` derefs its char* → pass `g_mem+(STRSEG<<4)+si`; `FUN_1000_a370`=linear 0x1a370, seg-0xf69). Then `4754→d501` FSG parser (7-entry table `DAT_2000_a9e6`@0x2a9e6 stride-3-words, open DX host-ptr `&DAT_2000_39a9`=DGROUP:0x79a9), `459a`, `1631` (LOD models `18dc(i,0x20/0x10/8/0)`), `e4bb` (in-mission game loop, template 0x2d1d). **VOXEL-ENTRY PIN:** the raycaster is a template-0x2d1d paint method (sibling of `FUN_0000_82ee` radar text), invoked per-frame downstream of the `D32/C32.KLC` load in d501; exact FUN offset pins once d501→KLC lands (`.KLC/.SKY/.PAL` suffix table @ image 0x163de/0x16419). **HONEST:** the rendered LOADING frame is NOT landable this iteration (multi-patch: e459+d501+KLC+paint; the screen is transient into the non-static voxel loop) — the furthest STABLE bit-verified frame remains the BRIEFING; 152/153 unblock the cascade's entry gate. Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (BATTLES→OK = THE LEVEL/MISSION-EDITOR ENTRY): CLICKING OK ON THE SELECT BATTLE LIST OPENS THE MISSION BRIEFING SCREEN (`7088` dialog id=0) — the FLDCOMP panel with the selected battle's briefing text ("BLOODFEUD! / AS ARMENIAN FORCES PUSH INTO AZERBAIJAN…" from AZER1.FSW) + ACCEPT/CANCEL — and its first settled frame is AE = 0/64000 vs a genuine, reproducible DOSBox capture on BOTH native AND wasm, native↔wasm 0-diff (md5 `bc73e2000f40`), deterministic (5× serial + pattern-init byte-identical, crash-free). One new verify.sh flow → `bash tools/verify.sh both` = 18/18.** 1 new patch (151). `re_out/fist.c` pristine (`61453e42`); `make check` = 151 patches apply; native + wasm green. NO REGRESSION (17 prior flows PASS); `git status` clean after verify. **RECON — BATTLES→OK opens the BRIEFING, not the voxel editor directly:** `e891` = `mov ax,0; call 7088; jb 0xe931` — CANCEL→menu, ACCEPT→the in-engine LEVEL/MISSION EDITOR build (`e43f/4754/459a/1631/…/6ade`); the briefing is the editor entry's first sub-screen and this iteration's bit-verified frame. The briefing is a `7088` MODAL (id=0), SAME machinery as ABOUT (id=4)/REVIEW (id=2): SETUP `FUN_0000_7162` builds `<battle>.FSW` (plaintext briefing; `.FSE`=EAST), opens it (INT 21h AH=3D) → reads ≤0x1800 B into STRSEG:0x452c, text drawer `FUN_1000_6c2d` renders it via the proven `5591` drawstring. (The `.FSG` SELECT-BATTLE-list entry is the EDITOR's battle/map DATA — `D32/C32.KLC` map, unit/path chunks — loaded behind ACCEPT, not for the briefing.) **PATCH 151 (asm-verified vs `re_out/fist_dat_image.bin`):** 7162 base-losses — the default "NO ORDERS…" `rep movsb` (the reg-file SI/DI host-ptr slots `pcRam000f0008/000a` OVERLAP at 0xf0008/0xf000a → second write corrupts first → SIGSEGV; reconstructed with plain-local effective addresses) + the AH=3D open DX (asm `mov dx,0x8bed`, filename @ DGROUP:0x8bed; Ghidra emitted the host ptr → open '' fails; write the DOS offset — host-ptr-into-DOS-register class, patch 077/146); `6d00`/`6d0f` buffer clear+remap rebased to STRSEG:0x452c; `6d53` (id=0 button sprite) reconstructed as the exact mirror of `6d3d` (patch 131). e87a dispatch extended: after cb7c, `a85d<2` (OK)→`7088(0)`. READ-only (no write until the editor's own save). Flow **battles-ok** (`ref/battles_ok_native320.png`, non-circular DOSBox capture via `tools/refcapture_ok.sh 160 100 205 128`, 2 captures AE=0 → deterministic). **FRONTIER — the full editor (next deliverables):** ACCEPT→the map-view build (`4754/459a/…`) — recon whether it invokes the **VOXEL terrain renderer** over the .FSG's D32/C32.KLC map (the large subsystem); the 6 INT-33h editor tools (ADD WAYPOINT / CUT UNIT OR PATH POINT / TANK ADDED @ 0x14d37–0x14dc8); the editor round-trip (create→save `.FSG`→reload→sim, byte-identical). Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (SELECT A CAMPAIGN SCREEN): CLICKING CAMPAIGNS → OK OPENS THE CAMPAIGN MODAL SCREEN `FUN_0000_ec7e` — a left LIST of the 7 decrypted campaign names (TRAINING [red-selected] / OVERWATCH / CROSSED SWORDS / AEGIS / CERTAIN FURY / FIRE HAMMER / BURNING FROST), a right HARDWARE group (WESTERN [red] / EASTERN radios) and OK / CANCEL — and its first settled frame is AE = 0/64000 vs a genuine DOSBox capture on BOTH native AND wasm, native↔wasm 0-diff, deterministic. One new verify.sh flow → `bash tools/verify.sh both` = 16/16.** 1 new patch (148). `re_out/fist.c` pristine (`61453e42`); `make check` = 148 patches apply; native + wasm green. NO REGRESSION (15 prior flows PASS); `git status` clean after verify. **PATCH 148 (all asm-verified vs `re_out/fist_dat_image.bin`)** — the recurring class is the **b941 campaign-name table walk** `si=word[DGROUP:0xf941+bx]` → a DGROUP NEAR-OFFSET Ghidra dereferenced as a HOST pointer (fault `0xfffff941`); rebase to `g_mem[0x1c000+(uint16)si]`, the parsed name at DGROUP:(si+1). (a) **DAT_2000_b848 retyped undefined2→uint8_t** (ec7e's exit flag is a BYTE `movb [0xf848]`; the WORD store `b848=0` clobbered the adjacent HARDWARE-icon rect at DGROUP:0xf849; 0=idle/1=OK/0xff=CANCEL). (b) **`FUN_0000_ec7e`** — fix the b941 walk + reconstruct the CF-driven modal loop faithfully (mirror the proven cc33 loop, patch 137: poll `[DGROUP:0x40a]`=35a7 sets `g_fist_cf` per patch 073 → CF=1 repaints via 206f, CF=0 dispatches 1e4b until b848; exit CF=(b848==0xff)==CANCEL; f0f1 OK-submenu unreached on the first frame). (c) **`FUN_0000_ed28`** (list paint, cdc4-lesson) — Ghidra DROPPED the selection-bar colour (`al=0xf8 if row==[0x77d6]`→c60a) + glyph colour (`al=0x1f/0x20 from byte[DGROUP:0x77da+row]&0x80`→c684); reconstructed per-row bar + hardware glyph + NAME drawstring (`thunk_FUN_1000_5531`=f69:0x5e9b, patch 139) from DGROUP:(si+1). (d) **`FUN_0000_edc2`** (hit-test) — clickY at `[si+4]`, si=coord node `word[DGROUP:0x18e2]` (ccde class, patch 142). (e) **`FUN_0000_ed14`/`ee67`/`eeab`** (HARDWARE box + WESTERN/EASTERN radio-icon paints) — Ghidra dropped the c6b4 sprite/rect args (sig per patch 139): ed14→0x13e0@0xf849; ee67→(DAT_2000_2db4!=0?0x13c8:0x13d0)@0xf812; eeab→opposite@0xf81a. (f) **`FUN_0000_ef39`** (keyboard accel, off the mouse-flow, reconstructed faithfully); **radios ee7c/eec0 + OK eeef/CANCEL ef14 VERIFIED faithful** (no change). (g) **e78c dispatch** (state 0x0): after cb74 OK (a85d<2) copy name (e9bd) + read .FPL (612c/6a9c), then call ec7e. Flow **campaigns-ok** (`ref/campaigns_ok_native320.png`, non-circular DOSBox capture via `tools/refcapture_ok.sh 160 87 205 128`, write-isolated). **FRONTIER:** the ec7e-OK tail (`6ade` write + `f338`/`7088` = the deeper campaign MISSION-SELECT via the `f0f1` submenu parser), then the **BATTLES mission EDITOR** (`e891`). Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (SELECT PLAYER "OK" SAVE PATH + WRITE-ISOLATION): CLICKING OK IN THE SELECT PLAYER LIST DIALOG RUNS THE REAL PLAYER-PROFILE SAVE (read+checksum+write the `.FPL`), copies the chosen name into CURRENT PLAYER, and returns to the menu drawing "CURRENT PLAYER:<name>" — AE = 0/64000 vs genuine DOSBox refs on BOTH native AND wasm, native↔wasm 0-diff, deterministic. Two new verify.sh flows → `bash tools/verify.sh both` = 15/15** (the 13 prior + **selplayer-ok** + **selplayer-ok-kkr**). 1 new patch (146); `re_out/fist.c` pristine (`61453e42`); `make check` = 146 patches apply; native + wasm green. NO REGRESSION (13 prior flows PASS). **`git status` stays CLEAN after a full verify run.** **PATCH 146 — the OK tail at the state-0xe handler 0xe97c** (asm-verified vs `re_out/fist_dat_image.bin`): `call cb74; jae 0xe9a6 (OK, a85d<2); e9a6: call e9bd; lcall f69:6a9c; lcall f69:6ade; jmp e714`. Three reconstructions: **FUN_0000_e9bd** copies the selected 8.3 name (DGROUP:0x77bc, written by cb74 on OK) into CURRENT PLAYER `ES:0xbf1` (ES=STRSEG=word[DGROUP:0x70], ≤8 chars stop at '.'/' ', `movb [0x6dad],1`) — Ghidra DROPPED the ES=STRSEG base on the DEST (raw 0xbf1) → rebased to STRSEG:0xbf1. **FUN_1000_612c (6a9c read+checksum)** + **FUN_1000_616e (6ade create+write)** set the INT-21h reg-file DX slot via `puRam000f0006 = &DAT_2000_37bc/37ce` — a HOST pointer stored into a 16-bit DOS register (R_DX=low16(hostptr)) → AH=3D/3C opened garbage; asm `mov dx,0x77bc`/`0x77ce`, R_DS=0x1c00 → store the real DOS offsets (**host-pointer-into-DOS-register reg-file artifact, patch-077 class**, the ~10-site class noted below). On OK the e714 state-0xe97c case runs e9bd+612c+616e; on CANCEL (a85d>=2) unchanged (menu-idle, patch 143). **WRITE-ISOLATION (harness only, engine keeps its faithful write):** 6ade is a REAL AH=3C/40 create+write of the `.FPL` under FISTDATA/. Both port (`$FIST_DATADIR`, fist_dos.c `datadir()`) and DOSBox (mounted dir) resolve paths at the harness boundary → `tools/verify.sh` runs WRITE flows (`WRITEFLOWS`) against a FRESH `cp -a` scratch copy of `armoredfist/` (deterministic, repo never mutated); `tools/refcapture_ok.sh`/`refcapture_ok3.sh` mount the SAME fresh copy for the DOSBox ref. **Flows: selplayer-ok** (default player D → "CURRENT PLAYER:D"; 6a9c checksum matches → 6ade rewrites D.FPL byte-identical; `ref/selplayer_ok_native320.png`) and **selplayer-ok-kkr** (row-select KKR → OK → "CURRENT PLAYER:KKR"; exercises the non-default name copy `e9bd` from STRSEG:(a838*0x10+0x3776) + a different `.FPL`; `ref/selplayer_ok_kkr_native320.png`). Both AE=0 both targets, non-circular genuine DOSBox refs. **FRONTIER:** (1) **CAMPAIGNS OK → the campaign MISSION-SELECT screen `FUN_0000_ec7e` is a LARGE unreconstructed screen (deferred).** Experimentally wiring it SEGVs at fault-addr `0xfffff941` = **`DAT_2000_b941` (DGROUP:0xf941) dereferenced as a host pointer** — ec7e's campaign-name table walk + `ef5e` (A.CAM/B.CAM… loader) + `f0f1` are all base-lost; a full multi-base-loss + cdc4-lesson paint-method reconstruction on the SELECT PLAYER-dialog scale (patches 137-139). BATTLES OK → `e891` mission EDITOR is bigger. (2) **Cursor ghost / scroll-release** — unchanged (the OK flows return to the REBUILT menu, so the menu→dialog cursor ghost does NOT appear on the OK path; both OK frames AE=0). Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (LIST-DIALOG SCROLLBAR): THE SCROLLBAR IS INTERACTIVE + BIT-VERIFIED TWO WAYS — (1) holding the DOWN ARROW auto-repeats to the clamped bottom (patch 144); (2) ONE click on the TRACK below the thumb PAGES DOWN 8 rows (patch 145) — both AE = 0/64000 vs genuine DOSBox references on BOTH native AND wasm, native↔wasm 0-diff, deterministic. Two new verify.sh flows → `bash tools/verify.sh both` = 13/13** (the 11 prior + **battles-scroll** + **battles-page**). 2 new patches (144/145); `re_out/fist.c` pristine (`61453e42`); `make check` = 145 patches apply; native + wasm green. NO REGRESSION (11 prior flows PASS). **PATCH 145** — clicking the track SIGSEGV'd (fault-addr 0xe8ed = element node 0xe8e9+4 as a HOST pointer): `FUN_0000_ce37`, the scrollbar TRACK/THUMB activate (ax=flags, bx=elem DGROUP offset, si=drained coord node at word[0x18e2], click Y at [si+4]), was base-lost the exact way patch 142's ccde was; control flow kept 1:1, memory model rebased. asm-verified 0xce37..0xcef3: press → clickY<y0+a86f+2 `d135` PAGE UP / >y0+a86f+8 `d146` PAGE DOWN / else THUMB GRAB (anchor a871, a86c=0); drag → remap through the 32-bit denominator a87d → a83a + `cef4`; release → a86d=0+dirty. Flow **battles-page**: track click (172,115) → CYPRUS2..INDIA2, red bar on CYPRUS2 (`ref/battles_page_native320.png`, two-click DOSBox capture, non-circular). **Flow — battles-scroll:** BATTLES (row 100) → SELECT BATTLE `.FSG` list (47 entries) → PRESS-AND-HOLD the down-scroll arrow (172,135) → the game's own timer-throttled auto-repeat clamps the view at the bottom (offset 39 → **UKRAINE1-8**, red bar on UKRAINE1). The clamp is a **deterministic endpoint independent of the iteration/tick count** (any ≥39 steps land the identical frame), so native/wasm/DOSBox all settle the same despite the modal loop's wall-clock-bounded iteration count. Ref (`ref/battles_scroll_native320.png`) captured **mid-hold** via the new `tools/refcapture_scroll.sh` (screenshot while the button is still down, so the arrow's pressed state matches; port md5 `26c1cf67` ≠ ref `6300db4e`, non-circular). **PATCH 144 root cause (runtime-traced):** the cc33 modal loop's per-frame handler slot **a85b** (DGROUP:0xe85b, dispatched `ax=[0xe85b]; call ax`) is a **WORD near code-offset** (cc33 `mov WORD [0xe85b],0xd0d7`; cd8f down-arrow `mov WORD [0xe85b],0xd103`), but Ghidra typed `DAT_2000_a85b` **undefined4 (DWORD) — overlapping a85d/a85e** → the dispatch read `word|(a85d,a85e)<<16` (e.g. 0x00ffd103) and `fist_icall_far` mapped that bogus seg:off to the no-op trap; d0d7 (idle) is empty so it looked identical to the no-op (11 flows unaffected) but the down-arrow's 0xd103 ALSO trapped → scroll never dispatched. Fix: retype a85b to `undefined2` (word) so `(uint32_t)a85b` zero-extends to linear 0xd0d7/0xd103 and resolves via the fmap; init a85b to the near offset 0xd0d7 (asm) not a host pointer; **recover FUN_0000_d103** (asm 0xd103..0xd134: throttle `[0x452]-[0xe885]>8` then fall through to the tail Ghidra recovered as `FUN_0000_d107(param_1)` → `d103 = d107((uint16_t)([0x452]-[0xe885]))`, faithful) + register 0xd103 in the fmap. Retyping a85b leaves the idle dispatch byte-identical → zero regression. **FRONTIER:** (1) **cursor ghost** (direct SELECT PLAYER row-select AE=35): the dialog backdrop SAVE bakes the cursor in; needs the asm-verified cursor hide/erase bracketing the save (`2d6d`/`2e12` or INT-33h hide counter) — high regression risk (all 13 flows' final cursor pixels are bit-verified). (2) **OK path → deeper screens:** SELECT PLAYER OK `e9a6→e9bd (copy name to STRSEG:0xbf1, [0x6dad]:=1)→0f69:6a9c/6ade` is **player-profile FILE I/O** — 6a9c OPENs+READs the `.FPL` and **6ade CREATES+WRITES it** (a real `armoredfist/` filesystem side effect / determinism hazard), NOT the clean menu-refresh the handoff implied; CAMPAIGNS OK → `[0x6dad]:=1; 6a9c; ec7e; 7088` = campaign MISSION-SELECT (large new screen); BATTLES OK → `e891` editor build (enumerate dispatch targets first — the cdc4 lesson). (3) **scroll residuals:** UP-arrow auto-repeat (`d0d8`) + THUMB DRAG (ce37 drag path, reconstructed) dispatchable but not yet flow-verified; and a REAL found divergence — **releasing the down-arrow after the clamp resets the port's scroll (a83a/a838→0) while DOSBox keeps the scrolled view** (AE=561 released vs AE=0 mid-hold) — needs asm-verified root-causing (cd8f release branch / event order) before a released-state scroll flow. Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (OK/CANCEL DIALOG CLOSE): THE DIALOG CLOSE PATH IS FIXED + BIT-VERIFIED — clicking CANCEL closes the list dialog and restores the plain main menu, AE = 0/64000 vs genuine DOSBox references on BOTH native AND wasm, native↔wasm 0-diff, deterministic (5/5 serial single-md5, normal + pattern-init byte-identical). Two new verify.sh flows → `bash tools/verify.sh both` = 11/11** (the 9 prior + **cancel** + **battles-cancel**). 1 new patch (143); `re_out/fist.c` pristine (`61453e42`); `make check` = 143 patches apply; native + wasm green. NO REGRESSION (9 prior flows PASS). **The handoff premise "the close path re-runs the boot reloc appliers `5214→f842/f7c3`" was a MISDIAGNOSIS** — 5214 fires once at BOOT (from app_entry), never on the close path; the crash was **the shared cb74/cb7c TAIL itself** (two entry points into ONE body from cb82; root-caused with the patch-127 discipline: pattern-init deterministic repro + `FIST_SEGV_BT`, fault-addr `0x3776`, EIP in cb74). asm-verified: (1) `cba1 jb 0xcbcb` — CANCEL (CF=1 from cc33, a85d>=2) must SKIP the selected-name copy; Ghidra folded the branch to always-false so the copy ran on CANCEL; (2) the copy source is **STRSEG:(a838*0x10+0x3776)** (`mov es,[0x70]`), base-lost to the raw host pointer 0x3776 → SIGSEGV; dest = param_7 (caller's DGROUP dest, unchanged); (3) `push/pop [0x452]` (click-timestamp save/restore across the dialog) was rendered `DAT_1000_c452 = unaff_CS` (uninitialised garbage — the ~60%-flaky class; plain gdb masked it). Patch 143 threads `g_fist_cf`, rebases the copy, restores c452 faithfully in BOTH cb74 and cb7c (`cbd4` on the mission-load path carries the same idiom, unreached — documented latent debt). **Flows: cancel** (CAMPAIGNS row 87 → `.FPL` roster → CANCEL @205,140 → menu; `ref/cancel_native320.png` md5 `b522abc1`) and **battles-cancel** (BATTLES row 100 → `.FSG` 47-entry list via the cb7c front-end → CANCEL → menu; `ref/battles_cancel_native320.png` md5 `e06b7483`); both AE=0 both targets, port frame `f147108c` ≠ ref md5s (non-circular). **FRONTIER:** (1) **cursor ghost** on above-panel clicks (direct SELECT PLAYER row-select AE=35 at x159-166/y73-79): NEW facts — the erase is fine on the plain menu (move-only = 0 ghost); the dialog backdrop SAVE (`e38a(0xf790)`→0fbc) captures the frame WITH the cursor baked in at the click pixel and the per-frame restore repaints it; the faithful fix is the original's cursor hide/erase bracketing the save (`2d6d`/`2e12` or the INT-33h hide-counter path) — asm-verify before touching (every flow's final cursor pixel is bit-verified). (2) **OK path → deeper screens:** SELECT PLAYER OK tail `e9a6: e9bd copy-name; 0f69:6a9c refresh; 6ade`; CAMPAIGNS OK → `[0x6dad]:=1; 6a9c; ec7e; 7088` = campaign MISSION-SELECT; BATTLES OK → `e891` mission-editor build (enumerate every newly-reached dispatch target first — the cdc4 lesson). (3) **scroll arrows:** `FUN_0000_d103` (timer-throttled scroll stepper) still unrecovered + cc33's `a85b` host-ptr init (seed/fmap like patch-121's 0x19ae; fixed-tick hold determinism). Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (LIST-DIALOG INTERACTIONS): CLICKING A ROW INSIDE A LIST DIALOG MOVES THE RED SELECTION BAR, BIT-IDENTICAL — AE = 0/64000 vs genuine two-click DOSBox refs on BOTH native AND wasm, native↔wasm 0-diff, deterministic (normal + pattern-init byte-identical, crash-free). Two new verify.sh flows → `bash tools/verify.sh both` = 9/9** (…+campaigns-select+battles-select). 1 new patch (142) + a two-click XTest DOSBox capture (`tools/refcapture_click2.sh`); `re_out/fist.c` pristine (`61453e42`); `make check` = 142 patches apply; native + wasm green. NO REGRESSION (7 prior flows PASS). **The dialog-internal-click path is now faithful (all asm-verified vs `re_out/fist_dat_image.bin`):** the LINCHPIN was `FUN_1000_3566`+the hit-test `FUN_0000_1eb4` — the synthesized LEAVE(0x200)/ENTER(0x400) hover events must be written into the QUEUE NODE `3566` allocates (asm returns it in BX), not into the old hover ELEMENT (patch 128 clobbered a dialog CONTAINER's word0 cls=4→0x400, killing hit-test recursion into its buttons/rows); `3566` now publishes BX in the shim global `g_fist_ev_node`. Then the row-select `FUN_0000_ccde` (coord from the drained event node `word[0x18e2]`, not param_3=base), the SIGFPE fix `a87d` (32-bit DWORD scroll denominator, was WORD→`0x10000` truncated to 0→div-by-zero), the "no-repaint" fix `a8ee`/`a8f4`/`a8f5` (BYTE element dirty-masks, WORD typing clobbered the adjacent child-list HEAD word→container child-walk found no children), and the paint-base fix `209e`/`208a` (publish the live walk base in `g_fist_paintbp`; 208a had read the stale global `fafe`=MENU base while the dialog was up). **Two flows (non-circular, the menu-item pixel is INSIDE the dialog panel so no cursor ghost): campaigns-select** (CAMPAIGNS row 87 → SELECT PLAYER `.FPL`, click KKR; `ref/campaigns_select_native320.png` md5 `c2bdaf7b`) and **battles-select** (BATTLES row 100 → SELECT BATTLE `.FSG`, click AZER5; `ref/battles_select_native320.png` md5 `c0a23f4e`). **FRONTIER:** (1) direct SELECT PLAYER row-select leaves a 35-px CURSOR GHOST at the above-panel menu-click pixel (cursor not erased across the menu→dialog transition); (2) OK/CANCEL close → the menu-rebuild reloc-applier cascade `5214→f842/f7c3` SIGSEGVs (patch-091/127 class); (3) scroll arrows — `cd8f` reconstructed but `FUN_0000_d103` (timer-throttled scroll) unrecovered + a85b host-ptr init. `FUN_0000_cf6b`/`cc33`/`20d6`/`2043` (OK/CANCEL close path) are reconstructed but that flow is not yet bit-verified. Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (BATTLES + CAMPAIGNS, screens 6 & 7): BOTH REMAINING MENU ITEMS ARE BIT-IDENTICAL — AE = 0/64000 vs genuine DOSBox references on BOTH native AND wasm, native↔wasm 0-diff, deterministic (normal + pattern-init byte-identical, crash-free). Added as verify.sh flows 6 & 7 → `bash tools/verify.sh both` = 7/7 (mainmenu+about+settings+review+selplayer+battles+campaigns). ALL SEVEN main-menu items now open + render bit-verified.** 2 new patches (140/141); `re_out/fist.c` pristine (`61453e42`); `make check` = 141 patches apply; native + wasm green. **Both are the SELECT-list dialog (patches 137-139 chain) reached through NEW e714 tail-jumptable states, so each needed only a faithful dispatch case (asm-verified vs `re_out/fist_dat_image.bin`):** **BATTLES (menu row 100 → screen-state 0x2 → jumptable target 0xe87a = FUN_0000_e87a):** `e39f(); cb7c(".FSG", si=0x4509 "SELECT BATTLE", di=DGROUP:0x79a9)` opens the **SELECT BATTLE** list dialog — 47 `.FSG` files (AZER1-7/CYPRUS1-7/INDIA1-7/SAUDI1-7/SYRIA1-7/TRAIN1-4/UKRAINE1-8, DOSBox-ascending) + scrollbar + SIDE:WEST label + OK/CANCEL; `cb7c` is the SAME front-end as `cb74` (differs only by `movw [DGROUP:0xe82e],0` vs 0xffff) reusing the entire enumerate/sort/cc33-modal/paint chain → ref `ref/battles_native320.png` (md5 `bc67bd1c`, non-circular DOSBox click at row 100). **CAMPAIGNS (menu row 87 → screen-state 0x0 → target 0xe78c = FUN_0000_e78c):** a player must be chosen first, so on a fresh boot (`[DGROUP:0x6dad]==0`) it opens the **SELECT PLAYER** `.FPL` roster dialog — `e39f(); if([0x6dad]!=1) cb74(".FPL", si=0x451a, di=0x77bc)` — the SAME dialog as clicking SELECT PLAYER directly (cursor at row 87) → ref `ref/campaigns_native320.png` (md5 `2d3e1ca9`). Both dialogs are MODAL (cc33 spins until OK/CANCEL), so the FIRST FRAME is captured before any OK; the deeper flows — the BATTLES **mission EDITOR** build (`7088` + `e43f/4754/e006/d755/9d26/459a/e4bb/616e` after CF=0/OK) and the CAMPAIGNS mission-select (`ec7e/7088` after a player is picked, `[0x6dad]:=1`) — are later, separate deliverables. **THE EDITOR ENTRY (recon):** clicking OK on SELECT BATTLE (CF=0) runs e87a's tail = the big display-list build (`4754` build + `459a` + the `[DGROUP:0xe8/0x594/0x59c/0x5f4]` device-method dispatches) → the in-engine LEVEL/MISSION editor; that surface is untouched here. NO REGRESSION: the 5 prior flows PASS (menu `3a6ff1c5`, selplayer `8c72a1f7`, …), default boot unaffected, `re_out/fist.c` pristine. Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (SELECT PLAYER, screen 5): THE SELECT PLAYER LIST DIALOG IS BIT-IDENTICAL — AE = 0/64000 vs the genuine DOSBox reference `ref/selplayer_native320.png` on BOTH native AND wasm, native↔wasm 0-diff (identical md5 `8c72a1f7`), deterministic (serial single-md5 stable, normal + pattern-init builds). Added as the 5th passing verify.sh flow → `bash tools/verify.sh both` = 5/5 (mainmenu+about+settings+review+selplayer).** 1 new patch (139); `re_out/fist.c` pristine (`61453e42`); `make check` = 139 patches apply. **BOTH prior blockers were ONE defect: `FUN_0000_cdc4` (the scrollbar-TRACK paint method) ran base-lost with all four MGAVIDEO method-vector args dropped** — its `[0x60a]`=1091 fills used codegen-GARBAGE bx/al → a WILD inverted rect (rows underflow 65501) filled colour 0x30 WIPED the screen olive every frame (Blocker 1), and the garbage-dependent write target SEGV'd ~1/8 (Blocker 2; crash-free under pattern-init where 0xAAAA lands inside g_mem). Patch 139 (asm-verified): **cdc4** reconstructed (0x3e08-recover + the DGROUP:0xe873 scratch rect; NB **a875 is a WORD store — Ghidra's int accessor zeroed the adjacent a877**, the 104/105/123 store-width class); **0f1a** (MGAVIDEO save-buffer RESERVE) reconstructed — the cc33 dialog restores the TERRAIN-ONLY backdrop via e38a/e39f, but 0f1a's `lcall [DGROUP:0xcc]` device-alloc is uninstalled (patch-049/134 gap class) → no buffer → save/restore no-op'd → the menu-item bars persisted; resolve `FUN_1000_182a` directly + REUSE the reserved buffer (no per-frame leak); **thunk_FUN_1000_5531** (b631's name glyph loop) dropped the AL char to `[0x684]` → names rendered as identical blocks; **MGAVIDEO 0x10d1** (masked highlight fill, "does not return" 1091-class) reconstructed + b5a4 threads ax=0x000e (AL=red fill, AH=0 preserves the black glyphs) → the red selection bar. Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (SELECT PLAYER, screen 5): THE FULL LIST-DIALOG RENDER CHAIN IS RECONSTRUCTED + DISPATCHES CRASH-FREE (title bar, list box + 7 sorted names + red selection, 5 buttons, 2 scroll arrows, the container full-paint walker) — but the menu-art BACKGROUND does not composite, so SELECT PLAYER is NOT yet AE=0 and NOT added to verify.sh.** 1 new patch (138). **NO REGRESSION: `bash tools/verify.sh both` = 4/4 (mainmenu+about+settings+review, native↔wasm 0-diff, AE=0), `re_out/fist.c` pristine (`61453e42`), `make check` = 138 patches apply, native + wasm green, default boot rc=124 (alive, not SEGV).** The linchpin: every list-dialog element paint method is dispatched by the **209e dirty-walk via a NEAR call with the element node live in BX**, but the __allregs C dispatch passes NO args → the methods read codegen GARBAGE; 209e publishes the element to **DGROUP:0x3e08** (as every working paint method reads it) — recover it there. Reconstructed (asm-verified): **ccca→b652** (title bar: fill(8)+box(1)+box-inset1(4)+setpos+title, fixed the 52d1 int*→int16 scaling); **ccd8→b5a4** (list box: the whole visible-row loop Ghidra lost — item index=BX, scroll/cap-8 count, name-ptr scaling, `loop` terminator — + fill(8)/border(2) + per-row name via b631 + selection highlight `[0x60e]` colour 0xe); **cd1c/cd70/cf8e/cf44** (buttons/scroll-arrows: box outline + icon sprite via `[0x6b4]`=26a1); **b631** (STRSEG name-source base-loss); the MGAVIDEO box/line writers **119f/11ba/11db + the folded 1216(horiz)/124b(vert)** (rebase the DGROUP-offset rect + fb/rowtable/pitch bases, class 1091/patch 060, 1677/patch 136); **1e73** (the container FULL-PAINT `[+0x4872]` child walker — the base-lost host-pointer walk was the crash, not its leaf methods which are no-ops for a paint event). **FRONTIER — two deep cc33-list-dialog blockers (why AE≠0 ≈ 61811):** (1) the **menu-art background does not composite** behind the dialog (final frame flat olive, 15 fb indices) — ruled out as a fullscreen fill (only the harmless initial cursor-clear fires), a palette collapse (raw fb genuinely 15 indices), and a corrupted menu-bg descriptor (`0xf6cc.word0=0x55ac`, identical plain-menu vs selplayer); the colour-8 panel fill *lands* (7016/7854 px) yet the final capture is flat → a paint-composite / page / e38a-save-restore-timing interaction specific to cc33 (7088 modals don't hit it), root not yet isolated; (2) a **flaky (~1/8, garbage-dependent) SEGV in the selplayer click path under real init** (crash-free under `-ftrivial-auto-var-init=pattern`) — isolated to the selplayer click (the 4 verify flows + default boot are deterministic + unaffected). Full detail: `docs/stage1.md` (top). Prior status:

## Status (superseded) — **STAGE 3 (SELECT PLAYER, screen 5): the first file-enumerated LIST dialog OPENS + ENUMERATES CORRECTLY; the RENDER is the frontier (NOT AE=0, NOT a verify.sh flow).** 1 new patch (137) + the shim FILEMGR find-first/find-next (INT 21h AH=4E/4F, `re_out/fist_dos.c`). Clicking SELECT PLAYER (menu row 74 → screen-state 0xe → e714 dispatch `_tgt 0xe97c = e39f(); cb74(".FPL")`) opens the modal list dialog `cc33` and **enumerates the seven `*.FPL` player files in the exact DOSBox order: D, GAMESWIN, JO, JOE, KKR, PP, TRT** (verified vs the genuine reference `ref/selplayer_native320.png`; the engine's own bubble-sort `b6dd` sorts ascending, so the order is independent of the shim's enumeration order — the shim additionally sorts 8.3-name ascending to reproduce DOSBox's `DOS_Drive_Cache`). BATTLES (state 0x2, `.FSG` → mission EDITOR) is a larger, later target. **Reconstructed (patch 137, asm-verified):** e714 state-0xe case; **cb74/cb7c** (the ".FPL"/".FSG" search pattern is a 32-bit store `mov [0xe845],eax`; Ghidra typed the target WORD → truncated to ".F" → 0 matches); **b756** (the get-DTA→find-first→copy→find-next loop's three base-losses: DTA seg from the reg-file ES, list dest STRSEG:0x3776+cnt·0x10, name src DTA:+0x1e); **b6dd** (the bubble-sort — a83e/a840 were host pointers 0x3776/0x3786 → non-terminating pass); **cc33** (the modal loop — g_fist_cf threading + cooperative INT-8 pump, was mis-exiting to the menu); **2006/208a** (the recursive child-list walkers that fall into the 201a draw-walk / 209e paint-walk — same base-loss class as 201a/209e patch 056/057, reached for the first time by this dialog's container element); **1d84/1d8e** (the dialog-position + mark-dirty walker — whole walk was host-pointer-based + WORD/int mis-scaled). **FRONTIER (NOT AE=0): the list-dialog RENDER** — the title-bar `ccca`/`b652` (drops the rect into the driver box/border methods `[0x60a]`/`[0x61a]`/`[0x61e]`) + the MGAVIDEO rect/line writers `119f`/`11ba`/`11db`/`1216`/`124b` (bx=DGROUP rect offset, needs the `g_mem+0x1c000+bx` rebase like `1091`/patch 060), then the list-box text rows + red selection highlight, the scrollbar (arrows + thumb), and the NEW/OK/CANCEL buttons — each a base-lost element paint method + driver blit (comparable to the whole SETTINGS reconstruction). Clicking SELECT PLAYER reaches this unreconstructed render; no verify flow / default boot clicks it → the 4 flows stay clean. **NO REGRESSION: `bash tools/verify.sh both` = 4/4 (mainmenu+about+settings+review, native↔wasm 0-diff, AE=0 vs DOSBox), default boot rc=0, `re_out/fist.c` pristine (`61453e42`), `make check` = 137 patches apply, native + wasm green.** Full detail: `docs/stage1.md` (top). Prior status:
