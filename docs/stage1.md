...
## DONE-ish (STAGE 3 — THE CAMPAIGN MISSION-SELECT SCREEN f338 "INTRODUCTORY CAMPAIGN" IS FULLY RECONSTRUCTED AND RENDERS 99.8% BIT-EXACT (114/64000) vs a genuine DOSBox capture; the ONLY residual is the 4 mission MARKER boxes — a subtle c622 inner-ring shade + a tick-gated blink that breaks determinism. NO new verify flow yet (honest — not AE=0), NO regression: verify.sh = 16/16 PASS, re_out/fist.c pristine `61453e42`.) — 1 new patch (149, touches `fist.c` + `fist_mga.c`).**

**WHAT f338 SHOWS (genuine DOSBox 3-click capture: CAMPAIGNS@160,87 -> roster OK@205,128 -> campaign OK@203,159):** a full-screen "INTRODUCTORY CAMPAIGN" screen — a left column of mission rows (TERRORDOME / LA DIABLA / INFERNO / TRIAD), each a small map-thumbnail square with a state-coloured marker box; a right column of per-mission briefing text; the SELECTED mission's name + two embossed briefing lines at the bottom; ACCEPT / CANCEL.  Reached from ec7e-OK: `6ade` writes the player campaign `.FPL`, then `f338` opens (its own template 0x2d37, dispatch flag `ba56`).  The deeper ACCEPT->`7088` screen / mission-start cascade (e7dd..) is a SEPARATE deliverable.

**RECON VERDICT (correcting the handoff): f338 needs NO new subsystem** — the map thumbnails are baked into the full-screen background `.MRL` (loaded by the f3a3 enter method via the already-implemented `[0x560]`/`[0x564]` bg path); per-mission drawing is box/fill primitives + drawstring.  It is ABOUT/SETTINGS-class arg-threading + base-loss work, NOT a voxel render.
