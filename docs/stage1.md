...
## DONE (this iteration) — **THE MAIN MENU IS PIXEL-EXACT (STAGE 2 CLEARED FOR THE MENU): AE = 0/64000 vs `ref/main_menu.png` on a normal run.** 4 new patches (119–122). `make check` = all 122 patches apply; default boot crash-free (rc=0, no SEGV); intro plays → menu renders bit-identical to the DOSBox reference (bg + gray box at row 62 + all 7 items + version string "AF-FD 1U10" + white cursor + palette). **Deterministic AE=0 across runs. NO REGRESSION.**

**MEASUREMENT PROTOCOL (important — the earlier "AE=11685" was a mis-capture).** The stable menu appears ~20 s in at `FIST_TICK_HZ=25000`. Capture it with the self-exit watchdog, NOT a mid-render SIGTERM:
`timeout -s KILL 45 env FIST_TICK_HZ=25000 FIST_RUNMS=22000 FIST_FBDUMP=/tmp/fb.ppm /tmp/fist_native`
then `convert /tmp/fb.ppm /tmp/fb.png` and `compare -metric AE fb.png <(convert ref/main_menu.png -filter point -resize 320x200 png:-)`. The old 11685 figure was a SIGTERM dump taken mid-enter (box half-drawn at row 31); with patch 119 alone the correct capture is already AE=204.
