...
## IN PROGRESS (STAGE 3 — SCREEN CRAWL, SCREEN 3/N = SETTINGS) — **THE WIDGET-SPRITE-SHEET FRONTIER IS RESOLVED: MSPRITE0.BIN (the common widget/dialog sprite sheet) now LOADS and REGISTERS into the global sprite directory, so the SETTINGS checkbox/radio boxes RENDER and the runaway 26de blit (the reported hang) is GONE. But a NEW, deeper finding corrects the prior status: the SETTINGS PANEL + TEXT + PALETTE do NOT actually render (the prior "panel/palette/text render" claim was WRONG — proven below). SETTINGS is on a FLAT-GREEN background with only the widget boxes; it is NOT bit-identical and NOT added to verify.sh.** 1 new patch (133) + the tools/oracle INT-21 open-trace (opentrace.c/trace_click.sh) + the genuine DOSBox reference `ref/settings_native320.png`. NO REGRESSION: `verify.sh both` = PASS mainmenu + about (2/2), menu md5 UNCHANGED `3a6ff1c5` (10/10 stable), about UNCHANGED. `re_out/fist.c` pristine (`61453e42`); `make check` = 133 patches apply.
