...
## DONE (STAGE 3 — LIST-DIALOG SCROLL ARROW) — **HOLDING THE DOWN-SCROLL ARROW IN A LIST DIALOG NOW AUTO-REPEATS THE LIST TO THE BOTTOM, BIT-IDENTICAL: AE = 0/64000 vs a genuine DOSBox reference on BOTH native AND wasm, native↔wasm 0-diff, deterministic. One new verify.sh flow → `bash tools/verify.sh both` = 12/12** (the 11 prior + **battles-scroll**). 1 new patch (144); `re_out/fist.c` pristine (`61453e42`); `make check` = 144 patches apply, native + wasm green. NO REGRESSION: all 11 prior flows PASS.
...
**FRONTIER (updated):**
1. **Cursor ghost** (direct SELECT PLAYER row-select AE=35) — unchanged; the dialog backdrop SAVE bakes the cursor in; needs the asm-verified cursor hide/erase bracketing the save (`2d6d`/`2e12` or the INT-33h hide counter) before touching (all 12 flows' final cursor pixels are bit-verified — high regression risk).
...
3. **Scroll UP + page + thumb** — the same a85b-word fix now makes ALL scrollbar dispatch resolvable; only the down-arrow auto-repeat is exercised/verified here. The UP arrow (`FUN_0000_d0d8`, already recovered), the page-up/down (`d135`/`d146`, recovered + in the fmap), and the thumb DRAG are reachable next.
