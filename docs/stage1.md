...
## IN PROGRESS (STAGE 3 — SCREEN CRAWL, SCREEN 3/N = SETTINGS) — **CLICKING SETTINGS NOW OPENS ITS FULL SCREEN: the screen-state machine, palette FADE-IN, panel + text-label + widget DISPATCH are all reconstructed and run; the panel/palette/text render. The frontier is the checkbox/radio SPRITES (the widget sprite-sheet is not loaded — directory[0x4f0] has no records at the sprite indices 0xe8/0xf0), which makes the sprite blit run away.** 1 new patch (132). NO REGRESSION: `verify.sh both` = PASS mainmenu + about (2/2), native↔wasm 0-diff, menu md5 UNCHANGED `3a6ff1c5`, about UNCHANGED. `re_out/fist.c` pristine (`61453e42`). SETTINGS is NOT yet bit-verified → NOT added to verify.sh (per the crawl rule: add only when it genuinely passes). No DOSBox reference captured yet (blocked on a stable rendered frame).
