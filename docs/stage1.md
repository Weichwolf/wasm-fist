...
## IN PROGRESS (STAGE 3 — SCREEN CRAWL, SCREEN 3/N = SETTINGS) — **THE SETTINGS SCREEN NOW RENDERS ESSENTIALLY BIT-IDENTICAL: AE = 1/64000 vs the genuine DOSBox reference on BOTH native AND wasm, native↔wasm 0-diff (identical md5 `8f084d6f`), deterministic. The textured metal panel, all group boxes, all labels, all checkbox/radio widgets (SKY/SMOKE/DETAIL/joystick/sound), the SETTINGS.MRL grayscale palette (settled fade), the status text, and the cursor are ALL bit-identical. The SOLE remaining delta is a single deterministic pixel — the trailing `.` on the status line ("NO JOYSTICK SELECTED" vs "…SELECTED.").** 1 new patch (134) + the `fist_vga.c` retrace fade-step. NOT yet added to verify.sh (AE=1 ≠ 0). NO REGRESSION: `verify.sh both` = 2/2, menu md5 `3a6ff1c5`, about AE=0; `re_out/fist.c` pristine (`61453e42`); `make check` = 132 patches apply.
...
**THE FRONTIER TO AE=0 (single deterministic pixel):** faithfully reconstruct `FUN_1000_444f` — seed the 0x2ac/0x2e0/0x2f0 method-vector family in `SeedServiceVecs.java`, re-decompile (pulls in 444f + 0xf69:0x482f/0x4507), install its reloc section (leading seg 0xf69, table @0x33720), and fix its base-losses; the real builder emits "NO JOYSTICK SELECTED." with the trailing period → AE=0 → add the `settings` flow to `verify.sh` (3/N). Until then SETTINGS is honestly NOT in verify.sh.
