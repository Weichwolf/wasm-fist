...
### Central azimuth-DIAL — RESOLVED to truth by the oracle (2026-08-18). See [[dial-needle-oracle-verdict]].
The 8 held-back missions (AZER6/CYPRUS5/INDIA7/SAUDI5/6/SYRIA5/7/TRAIN1) show a cockpit azimuth-needle
divergence vs their DOSBox refs. ~15 iterations of port-side hunting (all consolidated away here) chased
"port computes a wrong player veh+0x55; force it to ~56." **The instrumented-DOSBox oracle REFUTED that.**
Genuine original mission-spawn guest RAM (engine DGROUP relocated to guest phys 0x2d190, player object at
offset 0xc05c — byte-identical to the port): AZER1=56, SAUDI5=56, **AZER6 = 13/21/14** (a low band, NOT 56);
the port's 21 is INSIDE it. object+0x55 = a direct copy of DCBS record[0x55] (d81e), loaded faithfully; the
skipped PINF/BINF/TERM/STMP chunk handlers only read to scratch (no +0x55 write). **+0x55 is the DYNAMIC
turret azimuth: the original SLEWS it per-frame; the port FREEZES it** (FIST_V55WATCH: no transition). The
usi14 refs are frame-skewed settled frames → the 8 missions are NOT statically bit-verifiable and the refs
must NOT be trusted as spawn refs. **DO NOT patch +0x55 to 56** (would corrupt the faithful load). Real fix =
