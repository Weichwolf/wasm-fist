# AUDIO subsystem — recon + shim foundation (iterations 1–15)

Status date: 2026-07-18. Author task: stand up the first port audio output + the audio-verify method.

## 26. Iteration 20 — THE THREE POST-VOICING RESIDUALS RESOLVED/PINNED, measured against the CLEAN ref (`ref/audio_menu_oracle_clean.wav`). (1) TEMPO/PHASE is FAITHFUL (div=120 confirmed). (2) The "fnum semitone" is a MEASUREMENT ARTIFACT — pitches are ~85-90% faithful, NO fnum patch. (3) The WASM audio blocker is ROOT-CAUSED to a specific dropped-register-args reconstruction and FIXED by **patch 384** (asm-verified). Envelope xcorr vs the clean ref = **0.497** (vs the 0.22 the task cited against the OLD contaminated ref); ~0.67 at optimal tempo — the remaining gap is glide/synthesis waveform structure, NOT tempo/pitch/voicing. (2026-07-20)

**METHOD.** Compared the port's OPL register stream (`FIST_OPL_REGLOG`) and rendered WAV (`FIST_AUDIO_WAV`) against the oracle capture `/tmp/fistopl2` (menu window t≥26000 ms; the contaminated boot capture is CLEAN after t≥26 s where the note-on channel distribution flips ch1-dominant) and the clean ref. Note-on = the 0→1 transition of the B0 key-bit per channel. Envelope xcorr = 20 ms-RMS envelope, wide-lag search, self-consistency-checked (ref-vs-ref peaks 1.0 at lag 0).

**FINDING 1 — TEMPO/PHASE is FAITHFUL; div=120 (60.26 Hz) re-confirmed vs the CLEAN ref.** Port loop period = 62.38 s (autocorr 0.986 over 255 s). A software tempo-stretch sweep vs the clean ref peaks at factor 0.996 (envelope xcorr 0.67) — i.e. the port is within **0.4 %** of the ref, BELOW the integer-ISR-sub-division quantization (div=120→60.26 Hz; the nearest steps ±0.8 %). So div=120 is the asm-grounded faithful pin; a finer tweak would be curve-fitting to sub-quantization noise. Oracle clean-window note-on rate 9.65/s ≈ port 9.40/s. The iter-17 pin (div=120 from the CONTAMINATED capture) HOLDS against the clean ref.

**FINDING 2 — the "port is +1 semitone sharp" is a MEASUREMENT ARTIFACT (alignment), NOT a bug.** A naive channel-sequence alignment matched port note i to oracle note i+1 (the channel pattern is near-periodic), which for this melody reads as a consistent +1.00-semitone offset (median +1.00 st over 94/103 notes). But the ALIGNMENT-FREE per-channel pitch-SET comparison shows the port shares the oracle's exact 12-TET pitch grid: ch0 13/15, ch1 16/18, ch2 11/12, ch5 18/22 oracle pitches present in the port (the port sees 7 loops vs the oracle's 0.6, so its sets are supersets). The port's runtime state is byte-identical to the oracle's: fnum table `[0x9dd]` (`0083 0087 008a…`), transpose `[voice+0xd6]`=0, base note `[voice+0xc2]`=note×2, all match. **NO fnum patch is warranted** (consistent with iter-19: voicing is faithful). The 0a28 pitch-envelope glide adds a few chromatic passing notes (glide-phase), the residual waveform-structure gap.

**FINDING 3 — the WASM audio blocker: ROOT-CAUSED + FIXED (patch 384).** Under FIST_SB, wasm produced ZERO OPL writes (seq-advances=0, isr-runs=0) while native played. The SOUNDDVR device-registration gate in `thunk_FUN_0000_0078` (`*(short*)(g_mem+0x1c000) == 0xcbc3`, the engine DGROUP:0 signature) FAILED on wasm — DGROUP:0 held **0x3a06** (the SOUNDDVR load seg) instead of **0xcbc3**. Traced (`FIST_DG0_TRACE`/`FIST_184B_TRACE`/`__builtin_return_address`): `FUN_0000_f7ad`'s `call [ds:0x36]` (= the DGROUP:0x36 template-copy service `FUN_0000_f98b`) reconstruction DROPPED the four register args set up right before the call (asm 0xf7ad-0xf7b9: `ax=0x3e0` count / `bx=0x74` slot / `si=0` srcoff / `dx=[ds:0x74]` srcseg). f98b stores the allocated seg to `DGROUP:[slot]`; with the slot dropped it read a GARBAGE register — `(uint16_t)`-truncated to **0xcee8 on native** (harmless: a scratch DGROUP offset) but **0x0000 on wasm** (g_mem not 64 KiB-aligned → the host-ptr-truncated-to-near-offset wasm class, cf. the `wasm-mission-op50` memory note + patch 364). slot=0 → f98b wrote the alloc seg over DGROUP:0, clobbering the 0xcbc3 signature → the SB gate bailed → no timer-ISR → no music. **PATCH 384** threads the four args faithfully (slot is now the asm-specified constant 0x74 on BOTH targets), so the alloc seg lands at DGROUP:0x74 (the double-buffer field) as the asm mandates, never DGROUP:0.

**VERIFICATION (patch 384).** asm-verified vs `re_out/fist_dat_image.bin` @ 0xf7ad. Native: mainmenu md5 **3a6ff1c5** UNCHANGED (no video regression); the SB device registers (`[snd] 107a` fires, work object found) → native audio path intact. Wasm: the fix is DETERMINISTIC-BY-CONSTRUCTION (slot=0x74 is a compile-time constant, not a register → DGROUP:0 can never be clobbered on either target). Engine `re_out/fist.c` PRISTINE (`make check` = all patches apply; 384 is the only change). **CAVEAT:** empirical wasm-OPL re-confirmation + the full 36-flow regression were BLOCKED this session by an environment failure (the shared tmpfs was wiped mid-run, then the FIST_RUNMS watchdog hung on both native AND pristine builds — an environment issue, not patch 384). The follow-up should run `verify.sh both` + a wasm FIST_SB run once the environment recovers.

**VERDICT.** Voicing (iter-19), tempo/phase, and fnum/pitch are all FAITHFUL — the port already reproduces the song's notes, channels, tempo, and pitches. The bit-exact residual is the 0a28 glide-phase + DBOPL synthesis-phase waveform structure (envelope xcorr caps ~0.67 at optimal tempo), which is NOT a single fixable bug. The WASM dispatch (the last-mile gate for an `audio-menu` native↔wasm-0-diff flow) is fixed by patch 384. An `audio-menu` verify flow was NOT landed (bit-exact vs the oracle is not reachable; a native↔wasm-0-diff OPL-register-stream flow becomes possible once patch 384's wasm OPL is empirically confirmed).

## 25. Iteration 19 — THE VOICING RESIDUAL IS A PHANTOM: the port's menu-music voicing is ALREADY FAITHFUL (voice=channel), and the "redistribution" that iters 16/17/18 chased was a MEASUREMENT ARTIFACT — the iter-17/18 oracle capture (`/tmp/fistopl2`, `trace_opl.sh … 42 22`) started at BOOT and mixed the INTRO audio (a spread distribution) with the menu music. A FRESH clean-window capture proves the pure menu-music note-on distribution is BYTE-FOR-BYTE the song's channel distribution = the port's. No engine/driver patch is warranted (a 0c39 / layer-chain "fix" would be WRONG — the code is already faithful). The contaminated `ref/audio_menu_oracle_clean.wav` is REPLACED with a pure-menu reference. RECON + reference-fix only; 4 hard md5s + all shim UNCHANGED; mainmenu AE=0. VERDICT: the voicing sub-problem is CLOSED; the real residuals are tempo/phase-pin + fnum/envelope bit-exactness + wasm dispatch. (2026-07-20)

**FINDING 1 — the port's sequencer tables are BYTE-IDENTICAL to the oracle's (iter-18's "garbage `[0x20]`" was a pre-song-load transient snapshot).** Runtime-dumped the driver DS (`ds=[cs:0x831]`, base `fist_snd_base`) at the moment the song is actually loaded (`[ds:0x6]`=song seg 0x4c24 = MAINMENU.MS3 magic): `[0x20 link]`=`00 00 00 00 00 09 00 00 00 00 …`, `[0x2a prog]`=`5c 5b 60 58 5e 5a 5a 5a 26 5e`, `[0xc01 vmap]`=`00 01 02 03 04 05 06 07 63 08` — every byte matches the oracle RAM dump (`/tmp/fistopl2.ram.bin` @ DS=0x4d55). Iter-18 read `[0x20]` at the FIRST `[ds:0xe]!=0` tick, which fires on a PREMATURE register (`[ds:0x6]`=0, song not yet loaded) → it read low-memory CODE bytes (`fa 4a 42 9a 0e 00 …`), mistaking a transient for the steady state. `[0x20]` has exactly ONE writer in the driver image (`0b10` @ 0xb29, `mov [bx+0x20],al` from `song[bx+0x10..]`); `[0xc01]`/`[0x13b]` have ZERO writers (static init-data). So the tables are STATIC and identical — `[0x20]` is NOT dynamically rebuilt (0c94/0ca9 are stop/all-notes-off, they do NOT touch `[0x20]` — the iter-18 "per-section rebuild" premise is FALSE).

**FINDING 2 — the port's `0b5d` decode is byte-identical to the real driver's (decisive alignment test).** The oracle RAM snapshot has cursor `[0xc]`=0x818 with per-channel note table `[0x16]`=`33 00 30 00 00 54 30 24 00 00` (ch0/2/5/6/7 active). Decoding MAINMENU.MS3 forward to cursor 0x818 reproduces `[0x16]` EXACTLY. So the KGF'91 event/delta decode (patch 359's `0b5d`) is faithful, and the `0c39` chain-walk is faithful (runtime-verified: `evch=5` dispatches voice 5 AND the `[0x20+5]=9→ch8` layer, which is then faithfully dropped by `[0xc01+8]=0x63` in `10e3`'s `cmp ch,8; ja` — same as the oracle). The §24 "concrete `0c39` bug" (`[0x20+param_4>>8]`) was in the PRISTINE decompile and is ALREADY superseded by patch 359 — no fix needed.

**FINDING 3 — voice=channel is CORRECT: the pure menu-music oracle distribution = the song = the port.** Every OPL key-on in the oracle (588/588, backtrace-classified) routes through `0c39→0aa7→10e3` (0 via the `0997` SFX voice-allocator). With static identity `[0xc01]` and the minimal `[0x20]` chain, `0c39` dispatches OPL_voice = MIDI_channel — exactly what the port does. The OLD capture's aggregate `{0:102,1:151,2:71,3:32,4:14,5:90,6:78,7:48,8:2}` looked "redistributed" ONLY because it concatenated ~26 s of INTRO audio (the ch3/5/6-heavy spread) with the menu music. FRESH capture `trace_opl.sh … 48 70`, restricted to the post-intro window (t>55 s, one loop, 582 note-ons) → `{0:118,1:243,2:69,3:6,4:2,5:63,6:38,7:42,8:3}` = the song `{0:118,1:244,2:69,3:6,4:2,5:63,6:38,7:42,9:3}` (ch9→v8) BYTE-FOR-BYTE. Proof the "spread" is the intro, not menu music: the song has only 6 ch3 events TOTAL/loop, yet the old capture's first half showed ch3=30 — impossible for menu music, trivial for a different (intro) stream.

**REFERENCE FIX.** `ref/audio_menu_oracle_clean.wav` was built by iter-18 from the CONTAMINATED `/tmp/fistopl2` (intro+menu) → it is NOT a valid menu-music gate reference. Replaced with a pure-menu reference: `trace_opl.sh <pfx> 48 70`, trim the capture to the post-intro menu window (start at the ~26 s instrument-setup burst, one 63 s loop, rebase `t=`), replay through `oplreplay` → 61.7 s, note-on dist matches the song within loop-boundary noise (580 note-ons). raw-sample xcorr stays low (~0.02) vs the port — EXPECTED: raw xcorr needs bit-exact tempo/phase/fnum, which the voicing being correct does NOT provide. The voicing dist is the right metric and it MATCHES.

**VERDICT — the menu-music VOICING sub-problem is CLOSED (no patch; the port is faithful).** Iters 16/17/18 spent three iterations chasing a redistribution that does not exist in the driver — it was intro audio leaking into the reference. The remaining bit-exact residuals are the SEPARATE, real ones: (a) tempo/phase-pin (§22/23, div=120≈60 Hz — verify the port note-on rate 9.38/s vs the CLEAN menu ref, not the old one); (b) fnum/envelope timing (0a28 pitch-envelope glide); (c) wasm sound-dispatch (call_indirect, §23 #3). NOT bit-exact yet, but the voicing phantom is dead — do NOT re-chase 0c39 / [0x20] / [0xc01] / 0997.

## 24. Iteration 18 — THE ITER-17 PREMISE IS A CATEGORY ERROR (0x10cce is engine MEMMGR, NOT a voice allocator) + the runtime voice map is IDENTITY (the port is correct there) + a CLEAN phase-pinned reference is built and committed. The bit-exact blocker is RE-ROOTED: it is the dynamic per-section LAYER/CHAIN redistribution in the sequencer (0c39/0af4/0aa7/0b5d), NOT a missing allocator. RECON-ONLY (no engine/driver patch, no shim change) — the 4 hard md5s UNCHANGED; mainmenu AE=0 native (video path untouched). VERDICT: CHECKPOINT. (2026-07-18)

**FINDING 1 — `0x10cce` is the engine MEMMGR block-reformat, NOT the dynamic voice allocator (disassembly-proven vs `re_out/fist_dat_image.bin`).** The iter-17 hypothesis ("residual #2 = the voice allocator installed by the device-registration REFORMAT `0x10cce`, the `FUN_1000_1917` tail the shim SKIPS") is wrong. `1917` (linear 0x11917) = `push ax; call 0x11345 (owner-tagged MEMMGR search); jb fail; mov ds,ax; call 0x10cce; lret`. Its near-call target `0x10cce` (= seg 0x1000:0xcce) disassembles as a pure MEMMGR heap op: `cmp ax,[0x4]` (size), free-list counters at `ss:0x16dc`/`ss:0x16e2`, block-flags `movw $0x8082,es:0x2` / `orw $0x8020,[0x2]`, unlink `0x10d70`, coalesce `call 0x11114`. ZERO OPL/voice content. It resizes/splits the found work-object block; it does not populate any voice table. (The existing `fist_sb.c` shim comment already said "format (0x10cce) is the block reformat … the found block is already MEMMGR-live" — that was correct; iter-17's re-attribution to a voice allocator was the error.) **Reversing/wiring 0x10cce does nothing for voicing.**

**FINDING 2 — the runtime channel→voice map `[DS:0xc01]` is IDENTITY; "voice = MIDI channel" (what the port does) is the CORRECT base map, not a bug.** The three driver note-dispatch fns `0f99`/`10a6`/`10e3` read `voice = byte[DS:0xc01 + channel]` (disp-scan of the driver image: exactly 3 refs, ALL reads, NO writer anywhere). Read from the oracle's own RAM dump (`/tmp/fistopl2.ram.bin`) at the sequencer's runtime DS (`[cs:0x831] = 0x4d55`): `[0xc01] = 00 01 02 03 04 05 06 07 63 08 63 …` → ch0-7→v0-7, **ch9→v8**, ch8/≥10→0x63 (dropped). So the map is essentially identity. (The static image bytes at 0xc01 are CODE — the driver's runtime DS is a SEPARATE data block, so reading the image at 0xc01 is meaningless; use the RAM dump.) The fnum table `[0x9dd]` (12-TET OPL F-numbers `0083 0087 008a … 00ca`) and per-voice tables `[0xbe6]`/`[0xbf8]`/`[0x1dd]` are likewise in that runtime DS block.

**FINDING 3 — the port ALREADY plays the raw song with voice=channel; the oracle REDISTRIBUTES it (measured).** Note-on distribution across OPL channels (real key-ons = `B0|0x20` preceded by a `0x40-0x55` level write, per §23 FINDING B):
| | v0 | v1 | v2 | v3 | v4 | v5 | v6 | v7 | v8 |
|---|---|---|---|---|---|---|---|---|---|
| **oracle** (588 = 1 loop) | 102 | **151** | 71 | 32 | 14 | 90 | 78 | 48 | 2 |
| **port, per loop** (3186/5.4) | 119 | **246** | 69 | 6 | 2 | 64 | 39 | 43 | 3 |

The port's per-loop dist = **exactly the raw song MIDI-channel dist** `{0:118,1:244,2:69,3:6,4:2,5:63,6:38,7:42,9:3}` (§23) → the port maps voice=channel faithfully. The oracle SPLITS ch1's melody (246→151 on v1, the rest spread to v3+26/v4+12/v5+27/v6+40). The oracle's first-24 note-on OPL sequence `[7,0,2,3,5,6,6,7,2,5,6,…]` opens with NO ch1 at all; the port's `[0,1,5,8,1,1,0,2,1,…]` hammers ch1. So there IS a redistribution in the oracle — but it is **NOT via `[0xc01]` (identity in both)** and **NOT via `0x10cce` (MEMMGR)**.

**FINDING 4 — the redistribution mechanism is the sequencer's per-channel LAYER/CHAIN table `[0x20]`/`[0x2a]`/`[0x34]`, walked by `0c39` and rebuilt per song-section by `0af4`/`0c94`/`0ca9`.** `0c39` (asm 0xc7c-0xc8a): after dispatching a note on channel `ch`, it follows `ch' = byte[0x20+ch]; if ch'!=0 { ch = ch'-1; loop }` — **one song event fans out to a CHAIN of channels/voices**. The chain/patch tables `[0x20..0x2f]` (link), `[0x2a..0x39]` (program), `[0x34..0x43]` (0xff) are (re)loaded from the SONG data (`es:si`) at each section boundary by the `0aa7` 0xb10 branch / `0c39` tail (`0x20+i = song[si+i]` ×16, etc.). The captured oracle `[0x20]` at one instant = `00 00 00 00 00 09 00 00 00 00 …` (only ch5→ch8) — minimal, i.e. the chain is **dynamic per section**, so a single snapshot understates it. This is the real locus of the spread, and it is a MULTI-FUNCTION reconstruction (0b5d event decode incl. section events + 0af4/0c94/0ca9 layer rebuild + 0c39 chain walk + 0aa7 `[c1a5]` event dispatch), not a single missing allocator.

**CONCRETE LEAD (a genuine reconstruction bug in the port's `0c39` chain-follow).** ASM preserves `ch` across the `call 0xaa7` (`c73: push cx … c7b: pop cx`) and then follows `byte[0x20+ch]`. The reconstructed C (`re_out/fist_snd.c` `FUN_0000_0c39`) lost that preservation: on the dispatch path it sets `uVar3 = param_4` and reads `byte[0x20 + (param_4>>8)]` = `[0x20 + 0x0a]` (param_4=0xa37) instead of `[0x20+ch]`. It is MASKED today only because the port's runtime `[0x20+0x0a]==0` → the chain breaks after one dispatch → voice=channel with no fan-out. A faithful fix (patch: follow `[0x20+ch]`) is warranted but will NOT reach bit-exact alone, because the oracle's spread depends on the per-section `[0x20]` CONTENTS (rebuilt from song data), which the port populates differently — that rebuild path (`0af4`/`0c94`/`0ca9` reading the song's layer records) is the deeper gap. Fixing 0c39 without also fixing the `[0x20]` population risks sending notes to out-of-range channels (the port's higher `[0x20]` offsets hold string/garbage bytes, cf. the oracle's `0x5c` = "Copyright…") → deliberately deferred to a single layer/chain iteration.

**CLEAN REFERENCE — built + committed.** `tools/oracle/oplreplay.c` replays the iter-17 DOSBox OPL capture (`/tmp/fistopl2`, 3360 DATA writes over 62.8 s = one MAINMENU.MS3 loop, writer histogram = a single code path → no SFX pollution) through the port's own DBOPL → `ref/audio_menu_oracle_clean.wav` (62.8 s mono 44.1 kHz, peak 7963, rms 811). This is the usable bit-exact gate reference (the prior `ref/audio_menu_oracle.wav` is a 12.7 s stereo SFX clip — kept for history, do NOT use). Regenerate: `bash tools/oracle/trace_opl.sh /tmp/fistopl2 42 22` (needs the patched DOSBox, README_opl.md) → `/tmp/oplreplay /tmp/fistopl2 ref/audio_menu_oracle_clean.wav`.

**MEASUREMENT (port vs the clean reference, `tools/wavcompare.py` + a 20 ms-RMS envelope xcorr).** raw-sample xcorr **0.036** (≈ §23's 0.033 floor — unchanged, as expected: no code changed); 20 ms envelope xcorr **0.22** (best lag). Port peak 6160/rms 940 vs oracle 7963/811 — amplitudes are close; the WAVEFORM structure mismatches because the VOICING (which OPL voice each note lands on) differs per FINDING 3/4. The xcorr will not rise until the layer/chain redistribution matches.

**VERDICT — CHECKPOINT (report to the main loop).** The iter-17 reversal target (`0x10cce`) is a dead end (MEMMGR, not a voice allocator). The runtime voice map is identity (the port's voice=channel is correct). The bit-exact blocker is the sequencer's dynamic per-section LAYER/CHAIN redistribution (`0c39` chain walk + `0af4`/`0c94`/`0ca9` layer rebuild from song data + `0aa7` `[c1a5]` dispatch + `0b5d` section events) — a multi-function reconstruction, NOT a single allocator, and NOT curve-fittable. The clean reference + the measurement harness are now in place for that iteration. wasm sound-dispatch (call_indirect, §23 #3) remains the separate last-mile gate for an `audio-menu` verify flow; it is downstream of getting the voicing bit-exact on native first.

## 23. Iteration 17 — TEMPO RE-PINNED OBJECTIVELY (÷4.3) + THE VOICE-MAP RESIDUAL DECISIVELY DIAGNOSED. The menu-music note-on DENSITY now matches the oracle (9.38 note-ons/s, from the oracle's OWN OPL-write stream), correcting a ~4.3× tempo error the iter-16 WAV-xcorr sweep introduced. Residual #2 is PROVEN to be a dynamic VOICE ALLOCATION the reconstruction lacks — NOT tempo, velocity, or patch. Shim-only (`re_out/fist_opl.c` MUSIC_DIV 28→120 + a gated register-log); the 4 hard md5s UNCHANGED; 26 video flows unaffected (mainmenu md5 `3a6ff1c5`). (2026-07-18)

**METHOD (all from the oracle's actual OPL register stream, not by ear).** Re-captured the DOSBox OPL-writer oracle (`tools/oracle/trace_opl.sh /tmp/fistopl2 42 22` — clean 62.7 s menu window, 6720 writes) and added a full port register-log (`FIST_OPL_REGLOG=<path>` in `fist_opl.c`, gated, one line/0x389 write). Built `tools/oracle/oplreplay.c` = replay any capture through the port's DBOPL → a full-length mono 44.1 kHz **clean oracle WAV** (the committed `ref/audio_menu_oracle.wav` is a 12.7 s STEREO SFX-polluted clip — useless for xcorr).

**FINDING A — the port is FAITHFUL up to the voice map (verified vs `re_out/fist_snd_image.bin`):**
- Port **decode is exact**: the sequencer's decoded events (note/ch/vel/delta) match a from-scratch KGF'91 decode of MAINMENU.MS3 (585 note-ons, 585 note-offs, 3758 delta-ticks/loop; note-on chan dist `{0:118,1:244,2:69,3:6,4:2,5:63,6:38,7:42,9:3}` — ch1 = the melody).
- Port uses the **same fnum table** (D:0x9dd 12-TET) and **same AdLib init burst** as the oracle (byte-for-byte).
- 0b5d / 0c39 / 0aa7 / 0cf3 / 0a28 / 10a6 / 10e3 re-checked against the asm — the reconstruction is faithful. **0a28 does BOTH the sequencer advance AND a per-voice PITCH-ENVELOPE modulation** (note-stream at `[voice+0x90]`, adds a per-tick delta and reprograms the fnum via 10a6) — so the tempo clock drives both; one MUSIC_HZ fix corrects tempo AND envelope-glide rate.

**FINDING B — TEMPO was ~4.3× too fast; re-pinned to div=120 (60.3 Hz).** Distinguishing REAL key-ons (B0|0x20 preceded by a 0x40-0x55 level write, from 10e3) from 0a28 fnum-MODULATION B0-rewrites (preceded by an A0 write, which PRESERVE the 0x20 bit → the iter-12/16 "key-on" counts were ~2× inflated): the oracle emits **588 real note-ons ≈ 585 = exactly ONE MAINMENU.MS3 loop over 62.7 s** → delta rate 60.0 Hz, real note-on rate **9.38/s**. The port advances the sequencer once per music-tick, so MUSIC_HZ must be ~60 Hz → **div = 7231.4/60 = 120** (7231.4/120 = 60.3 Hz → 60.3×585/3758 = 9.38/s, oracle-exact). Prior div=28 (258 Hz) was a WAV-xcorr peak against the broken 12.7 s ref → spurious, 4.3× fast. A consistent 64 % real-keyon heuristic catch-rate confirms the port note-on rate = 9.38/s at div=120 (was 40/s). WAV **rms now 769 vs oracle 811** (was far lower); peak 5629 vs 7963.

**FINDING C — RESIDUAL #2 IS A DYNAMIC VOICE ALLOCATOR, decisively pinned (the frontier).** The oracle sequences byte-identical MAINMENU.MS3 (ch1-heavy note-ons) and **ALL 588 real key-ons dispatch via 0c39** (stack-backtrace classified; 0 via the 0997 SFX allocator) — yet the oracle's OPL-channel sequence is `[7,0,2,3,5,6,6,7,2,5,6,…]` (spread across voices 0,2,3,5,6,7, NO ch1 dominance), which is **not a subsequence of the song's** `[1,5,9,1,1,0,2,1,…]`. So the oracle maps each MIDI note to a FREE physical OPL voice (round-robin / voice-steal), while the port maps voice = MIDI channel (10a6/10e3 read the STATIC identity map D:0xc01, and 0c39's layer chain D:0x20 has no ch1 chain) → the port faithfully reproduces the SONG's channel dist (ch1 41.5 % = song 42 %) but the oracle SPREADS it (ch1 25.7 %, ch3/4/6 boosted). This spreading is the timbre/loudness gap (xcorr floor 0.033 vs clean oracle; peak 5629 vs 7963). The allocator is **NOT in the statically-readable device-3 path** (0c39→0aa7→10a6/10e3 all use voice=channel; D:0xc01 is read-only identity; 0997's free-voice search is on the 0966 SFX path). It is almost certainly installed by the **device-registration REFORMAT `0x10cce`** (the `FUN_1000_1917` tail the shim documented-SKIPS, §16) which populates a voice free-list / a non-identity voice→channel structure and/or the ISR-driven `[voice+0x111]` note-duration management. **NEXT: reverse 0x10cce + how it re-points the note dispatch to a free-voice allocator.** Until then the music plays at correct tempo/density but the wrong VOICING → not bit-exact.

**HONEST STATUS:** NOT bit-exact. Tempo/density FIXED (objectively, from the oracle stream); voice-allocation SPREADING is the pinned remaining blocker (needs the 0x10cce reformat reversed). Also still open: #3 wasm sound-dispatch (call_indirect); and the reference — `ref/audio_menu_oracle.wav` is unusable (12.7 s stereo SFX clip), so `oplreplay.c` synthesizes a clean full-length reference from a capture; a bit-exact GATE needs a phase-pinned clean oracle WAV committed.

**No-regression:** 4 hard md5s (`61453e42`/`0051cb56`/`75c6d726`/`e6d610c5`) UNCHANGED; `make check` passes; FIST_OPL/FIST_SB default-OFF → mainmenu native md5 `3a6ff1c5` (AE=0), video path unaffected. All changes are FIST_SB/OPL-gated shim (`fist_opl.c`) + tooling (`tools/oracle/oplreplay.c`).
...
- `0c39` decremented `_DAT_1000_c014` (engine DGROUP `0x1c014`, ==0) → `0xffff` → `!=0` → **returned every tick, never fetching** — cursor frozen at `0x30`, only 2 key-ons over 78 s.
- `0b5d` read the song via ONE near pointer `_DAT_1000_c00c` (engine DGROUP) instead of the FAR `es:[bx]` (`es=[ds:0x6]` song seg, `bx=[ds:0xc]` cursor) — wrong segment (==0), and its 14-bit variable-length delta decode (asm `0xbdc-0xbed`) was dropped (it only "skipped 1–2 bytes", never returned DX).

**PATCH 359 (asm-verified vs `re_out/fist_snd_image.bin`, 1:1):** reconstruct `0b5d` / `0c39` (incl. the `0xbf7` song-end/loop tail = `jmp 0xb10`) / `0cf3`, all based at the driver data seg (`DAT_0000_0831<<4`). `0b5d`'s x86 multi-register output — **AH** note / **CH** channel-or-`0xff` / **CL** velocity / **DX** delta (a multi-return the `__allregs` single return cannot express) — is threaded to the `0c39` note-dispatch loop via new file globals `g_snd_ev_note/ch/vel/dx`. The `0b10` song-init parse is factored into a shared static helper `snd_song_reparse()` so the loop tail re-registers the song identically to `0af4`'s initial register. NO re-decompile — the whole rebase is in the patch, so `fist_snd.c` stays `e6d610c5`.

**KGF'91 event format (asm-verified, `MAINMENU.MS3`):** header `4d 53 33 2d 4b 47 46 27 39 31 0a 0d 1a ff ff 02`; `@0x10` per-channel voice-layer config (`00 00 00 00 00 09 00…` — mostly 1 voice/channel, ch5→also voice 8); `@0x20` initial patch/note table (`5c 5b 60 58 5e 5a…`); `@0x30+` the MIDI stream. Event decode (from cursor C): status `es:[C]`, data `es:[C+1]`; high-nibble = MIDI status, low-nibble = channel. **NOTE-ON** `9X nn vv dd` (4 B): note→`[ds:ch+0x16]`, `dd`=delta. **NOTE-OFF** `8X dd` (2 B): vel→0, `dd`=delta. `cX pp dd` program (→`0cf3`), `eX bb dd` pitch-bend (→`[ds:ch+0xb8]`), `bX cc dd` control, `dX dd` pressure. Delta: 1 byte if bit7 clear, else 14-bit `((dl&0x7f)<<7)|next`. END = word `0x2fff`. One loop = **3758 delta-ticks / 585 note-ons**.

**Chain now closed (per PIT tick, FIST_SB/FIST_OPL):** `0a28` (envelope advance) → when `[ds:0xe]!=0` → `0c39` (decw `[ds:0x14]` delay; on 0 → fetch) → `0b5d` (read `es:bx` event, advance cursor, decode delta) → per note: `0cfb` instrument + `0aa7` key-on → device-3 method vectors `10a6` (A0/B0 fnum from `cs:0x9dd`) / `10e3` (key + velocity) / `0f99` (FM patch) → `0f21` OPL out → fist_opl.c/DBOPL synth. Trace confirms varied fnums (`0x8f`, `0x20b`, `0x83`…) with key-on bits — a real melody, not noise.

**HONEST — what plays vs what remains:**
- ✅ The native port plays a REAL `MAINMENU.MS3`-driven melody through the real sequencer + DBOPL. Menu run (fast-forward): 129267 OPL reg writes, 64423 key-ons, A0-A8=13132 / B0-B8=67132; WAV peak 4885 (oracle 6714), 23% non-silent.
- ⚠️ **CADENCE (follow-up #1):** tempo is **~16× too fast**. The shim (`native_main.c` line 433) drives the sequencer (`0a28→0c39`) once per **engine INT-8 tick**; the real driver reprograms **PIT ch0 to divisor `0xa5`=165 (7231 Hz)** with a down-counter (`05b6/05b8 = 65536/165 − 2 = 395`, ~18.3 Hz game-chain), and the music sequencer advances at its OWN music-timer rate (`≈3758 ticks / 12.74 s oracle ≈ 295 Hz`), NOT every sample-timer tick. The port's `9.24 samples/seq-tick` must become `~150` (advance the sequencer ~16× less often, or generate ~16× more samples per advance). This is a pure `fist_sb.c`/`fist_opl.c`/`native_main.c` cadence calibration — NO engine patch. Once pinned, `wavcompare.py` xcorr becomes meaningful (currently 0.0 — 104 s of looped, over-fast audio vs the oracle's 12.74 s).
- ⚠️ **VELOCITY/VOICE MAP (follow-up #2, likely folded into cadence):** RMS 249 vs oracle 1446 — notes fly by (no sustain) at 16× speed; re-measure after the cadence pin. The voice-layer loop writes the same fnum to consecutive OPL channels (config-driven layering) — validate the voice→OPL-channel map against the oracle after cadence.
- ⚠️ **WASM sound-dispatch (follow-up #3):** the melody milestone is native. The wasm `call_indirect` signature-normalization of the device-3 method-vector dispatch (`10a6`/`10e3`/`0f99` via `[ds:0x1a5]`/`[ds:0x1b3]`/`[ds:0x1c1]`) needs the same `-sEMULATE_FUNCTION_POINTER_CASTS` treatment the engine uses — untested with sound on. Video path (OPL off) is native↔wasm 0-diff.

**No-regression:** 3 hard engine md5s (`61453e42`/`0051cb56`/`75c6d726`) + `fist_snd.c` `e6d610c5` UNCHANGED. FIST_OPL/FIST_SB default-OFF → `fist_opl_owns()`==0 → 26 video flows byte-identical (`verify.sh both`), mainmenu AE=0 native+wasm, native↔wasm md5 `3a6ff1c5` (0-diff). Patch 359 reached only under FIST_SB (device 3 selected + `c510` populated via the FIST_SB-gated `014e` install); default boot never enters the sequencer.
